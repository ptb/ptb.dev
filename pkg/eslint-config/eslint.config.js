...
export const config = [
  {
    ...
    languageOptions: {
      ...
      parserOptions: {
        ...
        project: ["", "api", "app", "doc", "lib", "sql", "web"]
          .map(
            (path) =>
              existsSync(join(rootDir, path, "tsconfig.json")) &&
              join(rootDir, path, "tsconfig.json")
          )
          .filter(Boolean)
          .concat(join(rootDir, "pkg", "*", "tsconfig.json")),
